...
//
//-Hashing -> Transformation of a Stirng of characters to a shorted fixed length value that represents origonal String. A shorter value helps in
...
//
//How put works ->
//Hashmap comes with a zise of 2 raise to power n. Default size of HashMap is 16.
//put(K k, V v){
//	hash(k);
//	index=hash & n-1;
//}
//When we call put, firstly it computes the hash of the key and then the index is calculated so that we can find exactly we need to fit the value in
//out HashMap
...
//then the value at that node is returned. In the case of collosion we will match the hashmap if it does not matches we will shift to the 
...
// we will returen the value.

//In java-8 when we have to many unequal keys which gives same hashcode(index), when the number of times in a hash bucket grows beyond certain 
